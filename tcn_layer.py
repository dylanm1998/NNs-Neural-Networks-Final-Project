...
from keras.src.layers import Activation, SpatialDropout1D, Lambda, Layer, Conv1D, Dense, BatchNormalization, LayerNormalization
# from keras.utils.generic_utils import register_keras_serializable  # For previous Keras versions
from keras.src.saving import register_keras_serializable  # For last Keras versions
...
class ResidualBlock(Layer):
    ...
    def _build_layer(self, layer):
        """Helper function for building layer
           对每一个子层(Conv1D、Norm、Activation、Dropout)做手动 build, 并更新当前输出形状。
    ...
    def build(self, input_shape):
        ...
        with tf.name_scope(self.name):  # name scope used to make sure weights get unique names
            ...
            for k in range(2):  # dilated conv block. 两个时序卷积子块
            # 1) Conv1D -> Norm -> Activation -> Dropout
            # 2) 如果输入通道数与 nb_filters 不同，额外加一个 1×1 卷积作通道匹配
            # 3) 最后再加一个激活层作为残差输出
            # 4) 把所有子层都绑定到 self._layers，确保 Keras 能跟踪到它们
                ...
                with tf.name_scope('norm_{}'.format(k)):
                ...
                with tf.name_scope('act_and_dropout_{}'.format(k)):
            ...
            if self.nb_filters != input_shape[-1]: # 通道匹配
                ...
                name = 'matching_identity' # 恒等映射
                self.shape_match_conv = Lambda(lambda x: x, name=name)

            with tf.name_scope(name): # 残差融合 
            ...
            for layer in self.layers:
                ''' 将所有子层通过 self.__setattr__ 绑定到层对象上，以便 Keras 保存权重、序列化。'''
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        # 用通道匹配层对原始 inputs 做捷径 x2
        x2 = self.shape_match_conv(inputs)
        # 将 x1 与 x2 相加，并做一次激活，返回做下一块输入的残差输出 x1_x2，以及用于 skip connection 的 x1。
...
class TCN(Layer):
    ...
    def build(self, input_shape):
        ...
        self.residual_blocks = []
        # 堆叠残差块：共 nb_stacks × len(dilations) 个。
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        if self.go_backwards: # 可反向处理（倒序输入）。
            # reverse x in the time axis
            x = tf.reverse(x, axis=[1])

        self.layers_outputs = [x]
        self.skip_connections = []
        for res_block in self.residual_blocks: # 串行执行所有残差块，累计每块的 skip output。
            try:
        ...
        if self.use_skip_connections: # 若开启跳跃连接，将所有 skip output 相加。
        ...
        if not self.return_sequences: # 如果 return_sequences=False，就用一个 Lambda 取最后（或中间）的时间步。
